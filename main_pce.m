...
tlist = [5.5, 6.0, 6.5, 7.0, 7.25, 8.0, 8.25, 8.5];
...
Kig_lb = 0.005 * 0.9;
Kig_ub = 0.005 * 1.1;
vo_lb = 15*0.24/(0.024+0.24) * 0.9;
vo_ub = 15*0.24/(0.024+0.24) * 1.1;
P_lb = [vgmax_lb, Kg_lb, vzmax_lb, Kz_lb, Kig_lb, vo_lb];
P_ub = [vgmax_ub, Kg_ub, vzmax_ub, Kz_ub, Kig_ub, vo_ub];
...
metaopts.Degree = 1:20;                     % degree of polynomial. If array is given then automatically select one with lowest LOO error 
metaopts.TruncOptions.qNorm = 0.75;         % sparse truncation scheme
...
for it = 1:ntime
    ...
    if strcmp(myNS{1,it}.fit_type, 'ns')
        for idisc = ndisc:-1:1
            index = find(tdisc_pce{idisc}.Y <= tlist(it));
            if ~isempty(index)
                index_disc(idisc) = 1;
                break
            end
...
% Remove dfba integrator from path 
rmpath('./dfba_model')
